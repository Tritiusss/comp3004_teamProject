    ...
    , ui(new Ui::MainWindow)
{
    ...
    //maybe put all of this in an init records function and call it in the constructor? cleaner
...
void MainWindow::addRecording() {
    ...
    QStandardItem *curr = new QStandardItem(QString::number(current));
    records->setItem(numRecords,2, curr);

    QStandardItem *start = new QStandardItem(timeCountDown);
    records->setItem(numRecords,3, start);

    QStandardItem *duration = new QStandardItem(QString::number(currentTime));
